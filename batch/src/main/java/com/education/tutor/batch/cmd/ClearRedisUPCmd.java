...
@Component
@Order(value = 99)
public class ClearRedisUPCmd implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		if (cl.hasOption(Application.ClearRedisUPOption)) {
			logger.info("now clearing redis user principal");
			redisTemplate.keys("UP:*").forEach(key -> {
				logger.info("delete redis key " + key);
				redisTemplate.delete(key);
			});
			logger.info("finished clear redis user principal");
		}
	}
